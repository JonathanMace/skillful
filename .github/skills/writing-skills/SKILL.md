...
3. **Define the trigger before writing** — articulate the kind of request or task that should cause Copilot to load this skill. This becomes your `description` field and sets the scope boundary for the entire skill.
4. **Choose a placement** — project-level (`.github/skills/`) or personal (`~/.copilot/skills/`).
5. **Create a directory** named in lowercase with hyphens (e.g., `github-actions-debugging`).
6. **Author the `SKILL.md`** — write YAML frontmatter (`name`, `description`) then a Markdown body with instructions. Keep the body narrowly scoped to the trigger you defined in step 3.
...
- **Author custom instructions** (`.github/copilot-instructions.md`) for broad, always-on guidance like coding standards that apply to every task.
- **Author a custom agent** (`.agent.md`) when you need a specialized persona with constrained tools or a distinct approach.
...
Copilot CLI also reads skills from `.claude/skills/`, `.agents/skills/`, and their home-directory equivalents for compatibility, but `.github/skills/` and `~/.copilot/skills/` are the recommended locations.
...
name: my-skill-name
description: >-
  What this skill does and when Copilot should use it.
  Include trigger keywords that match likely user prompts.
license: MIT
---
```
...
|----------|----------|-------------|
| `name` | **Yes** | Unique identifier. Lowercase, hyphens for spaces. Must match directory name. |
...
4. **Keep it concise** — aim for 1–3 sentences (up to 1024 characters).
...
```yaml
description: >-
  Assists with writing and reviewing pull request descriptions.
...
The body contains the actual instructions Copilot will follow when the skill is loaded. Write these as if you are onboarding a skilled developer who has never seen your project.
...
You can include scripts, templates, and other assets in the skill directory and reference them in the instructions:
...
Copilot automatically loads a skill when it determines the skill is relevant to the current task, based on the skill's `description` matching the user's prompt. This is why the `description` field is critical — it controls discoverability.
...
7. **Test your skill** — after authoring a skill, run `/skills reload` and then prompt Copilot with a task that should trigger it. Verify it activates and produces good results.
8. **Keep instructions actionable** — author numbered steps, not vague aspirations. Tell Copilot exactly what tools to use and in what order.
